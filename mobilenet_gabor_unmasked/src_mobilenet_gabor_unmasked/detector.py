...
class PersonDetector:
    ...
    def __init__(self, similarity_threshold: float = 0.55,
        ...
        Args:
            ...
            combine_features: Whether to combine embeddings and Gabor features
        """
    ...
    def combine_embedding_gabor(self, embeddings: np.ndarray, 
                               gabor_features: np.ndarray) -> np.ndarray:
        """Combine embedding and Gabor features with normalization"""
        # Normalize each feature type
        embedding_norm = embeddings / (np.linalg.norm(embeddings, axis=1, keepdims=True) + 1e-7)
        gabor_norm = gabor_features / (np.linalg.norm(gabor_features, axis=1, keepdims=True) + 1e-7)
        
        # Concatenate
        combined = np.hstack([embedding_norm, gabor_norm])
        return combined

    def train(self, embeddings: np.ndarray, gabor_features: np.ndarray,
              labels: list, use_cross_validation: bool = True):
        """
        'Train' by storing average signatures for each person
        """
        if self.combine_features:
            features = self.combine_embedding_gabor(embeddings, gabor_features)
        else:
            features = embeddings
        ...
        if use_cross_validation:
            ...
            preds, _ = self.predict(embeddings, gabor_features)
    ...
    def predict(self, embeddings: np.ndarray, gabor_features: np.ndarray) -> Tuple[list, np.ndarray]:
    ...
    def predict_single(self, embedding: np.ndarray, gabor_feature: np.ndarray) -> Tuple[str, float]:
        embedding = embedding.reshape(1, -1)
        gabor_feature = gabor_feature.reshape(1, -1)
        labels, confs = self.predict(embedding, gabor_feature)
        return labels[0], float(confs[0])

    def evaluate(self, embeddings: np.ndarray, gabor_features: np.ndarray,
                 true_labels: list) -> dict:
        predicted_labels, _ = self.predict(embeddings, gabor_features)
